...
def add_entry(cls, sub, day, start):
    ...
    dur = 3 if sub.endswith("LAB") else 3 if sub in THREE_PERIOD_SUBS else 2 if sub in TWO_PERIOD_SUBS else 1
    if start + dur - 1 > 7:
...
if st.button("Download Excel"):
    ...
    with pd.ExcelWriter("Timetable.xlsx", engine="openpyxl") as w:
        ...
        for cls in df["Class"].dropna().unique():
            ...
            tt.to_excel(w, sheet_name=safe_sheet_name(cls, "CLASS_"))
        ...
        for fac in df["Faculty"].dropna().unique():
            ...
            tt.to_excel(w, sheet_name=safe_sheet_name(fac, "FAC_"))

        # ---- LAB TIMETABLES ----
        for lab in df["Lab"].dropna().unique():
            tt = grid(
                df[df["Lab"] == lab],
                lambda r: r["Class"]
            )
            tt.to_excel(w, sheet_name=safe_sheet_name(lab, "LAB_"))

        # ---- ROOM TIMETABLES ----
        for room in df["Room"].dropna().unique():
            tt = grid(
                df[df["Room"] == room],
                lambda r: r["Class"]
            )
            tt.to_excel(w, sheet_name=safe_sheet_name(room, "ROOM_"))
